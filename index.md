...
- We make **no warranty** about the accuracy, completeness, timeliness, reliability, or availability of any data shown in the App, including but not limited to sobriety counts, current streak, longest streak, attempts, daily check-ins, daily reflections, milestones, drink logs, body metrics, statistics, money-saved estimates (total or current-streak, including any continuously ticking counter), birthday cards, push notifications, local reminders, or streak-at-risk nudges.
- We make **no warranty** that any notification — push, local reminder, milestone celebration, friend-activity alert, or streak-at-risk warning — will be delivered, delivered on time, delivered in the user's preferred language, or delivered at all. Push delivery depends on Google Firebase Cloud Messaging, the device's network, the operating system's battery-optimization state, do-not-disturb settings, the user's notification permission, and the recipient's notification-channel settings — all outside our control.
...
- You are solely responsible for the **accuracy** of any data you enter (drink amounts, weight, dates, notes, profile information, country, language, monthly spending, currency, date of birth, daily check-ins, daily reflections, notification preferences, etc.).
...
### 3.4a Push Notifications and Reminders
- A **Firebase Cloud Messaging (FCM) registration token** is generated by Google Play Services on your device and stored on your account record. This token lets our Cloud Functions deliver notifications (friend requests, friend acceptances, your milestones, friends' milestones) to your device. The token rotates automatically; the App writes the latest value on every sign-in and on token refresh events.
- Your **device language code** (e.g. `en`, `hu`) is stored on your account so server-sent notifications can be delivered in that language.
- **Per-category notification preferences** (friend requests, milestone celebrations, friends' milestones, daily reminders, streak-at-risk) are stored on your account so the same preferences apply across devices.
- **Local reminders** (morning check-in nudge, evening check-in nudge, 9pm streak-at-risk nudge) run entirely on your device via Android WorkManager. They do not transmit data to us. Each reminder reads your local check-in cache and shows or skips the notification accordingly.
- We may use Cloud Functions running on Google Cloud to compose notification text (including replacing your name and milestone day count) and route it via FCM to the relevant recipient device(s). This processing happens automatically based on database events you trigger (e.g. sending a friend request, hitting a milestone day) and you cannot opt out of the processing while keeping the corresponding feature; you may, however, disable any notification category in **Settings → Notifications** at any time.
...
- You may **cancel, pause, change, or resubscribe** at any time via **Settings → Manage subscription**, which deep-links to the Google Play subscription management page for this App. The cancellation is processed entirely by Google Play; we do not control timing, refunds, prorations, or any aspect of the billing lifecycle. Subscription benefits remain active until the end of the current paid period; after expiry the App reverts to the free tier on the next entitlement check.
...
### 3.6a Android Permissions
The App requests the following Android permissions:
- **`POST_NOTIFICATIONS`** (Android 13+) — required to display push notifications and local reminders. You may deny or revoke at any time; the App degrades gracefully (no notifications shown).
- **`CAMERA`** — used only when you tap "Take a Photo" to set a profile picture. Optional.
- **`INTERNET`**, **`ACCESS_NETWORK_STATE`** — required for Firebase sync and ad serving.
- **`com.google.android.gms.permission.AD_ID`** — Advertising ID for AdMob in the free tier.
- **`com.android.vending.BILLING`** — required for Google Play subscription purchases.
...
- **Cloud:** Google Firebase (Authentication, Realtime Database, Cloud Messaging, Cloud Functions, Crashlytics, Analytics) on Google's infrastructure.
- **Push delivery:** Google Firebase Cloud Messaging delivers push notifications to your device via the operating system's push channel.
...
When the **Milestones** category is enabled, the App may send a push notification to your friends when you reach a milestone (e.g. 30 days sober). The notification includes your display name and milestone day count. Disable the **Milestones** sharing toggle to stop these outbound notifications. Each friend can also disable inbound friend-milestone pushes at their own end via **Settings → Notifications → Friend milestones**.
...
- You may delete your account at any time from **Settings → Delete account**. This permanently removes your profile, authentication credentials, FCM token, language code, notification preferences, daily check-ins, sobriety records, drink entries, body metrics, friendships, and friend requests. A server-side function cascades cleanup of edges held by other users (e.g. removes you from friends' lists). Local check-in / streak-at-risk reminders scheduled via Android WorkManager are cancelled on logout.
